...
import { SCHEMES, WORKOUTS } from '../utils/FitForge'
...
function Header(props){
    ...
    return (
        <div className='flex flex-col gap-4'>
            ...
            <p className='font-semi-bold text-lg sm:text-base mx-auto'>{description}</p>
        </div>
    )
}
  ...
  return(
    <SectionWrapper id={'generate'} header={"Generate your Workout"} title={['It\'s','Huge','o\'clock']}>
      ...
      <div className='bg-slate-950 p-3 border border-solid border-blue-400 rounded-lg flex flex-col'>
        <button onClick={toggleModal} className='relative p-2 flex items-center justify-center'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
                {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
                    return (
                        <button onClick={() => {
                            setGoal(scheme)
                        }} className={'bg-slate-950 px-4 border  duration-200 hover:border-blue-600 py-3 rounded-lg px-4 ' + (scheme === goal ? ' border-blue-600' : ' border-blue-400')} key={schemeIndex}>
