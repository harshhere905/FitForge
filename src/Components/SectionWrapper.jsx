   ...
   const {children, header, title,id} = props
    ...
    <section id={id} className='min-h-screen flex flex-col gap-10'>
      <div className='bg-slate-950 py-10 flex flex-col gap-2 justify-center items-center p-4'>
        <p className='uppercase font-medium font-semi-bold'>{header}</p>
        <h2 className='font-bold text-3xl sm:text-4xl md:text-5xl lg:text-6xl p-1'>{title[0]} <span className='uppercase text-blue-400'>{title[1]}</span> {title[2]}</h2>
      </div>
